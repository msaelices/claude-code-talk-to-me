...
@mcp.tool(name="get_transcript", description="Get the transcript of the current or last audio session")
...
@mcp.tool(name="end_call", description="End the current audio communication session")
...
@mcp.tool(name="test_audio", description="Test the audio system setup (microphone, speakers, TTS, STT)")
async def test_audio() -> Dict[str, Any]:
    ...
    """
    results = {"microphone": False, "speakers": False, "tts": False, "stt": False}
    ...
    return (
        ...
        else error_response("Some audio tests failed", results=results)
    )
